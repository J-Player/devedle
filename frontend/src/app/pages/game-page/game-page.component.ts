...
export class GamePageComponent implements OnInit, OnDestroy {
  ...
  giveUp() {
    ...
    if (this.game?.gameStatus !== GameStatus.IN_PROGRESS) return
    while (this.game.attempts.length < this.game.maxAttempts!) {
  ...
  reset() {
    const itemIndex = this.items.findIndex(i => i.id === this.game?.original.id)
    this.items.splice(itemIndex, 1)
    if (this.items.length === 0) {
      this.itemService.getAll()
      .pipe(take(1))
      .subscribe(items => this.items = items.filter(item => item.imageId !== null))
    }
    this.gameService
