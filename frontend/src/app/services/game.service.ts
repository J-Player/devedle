...
export class GameService {
  ...
  constructor(private readonly client: HttpClient, private readonly localStorageService: LocalStorageService) {}
  ...
  getGame(itemId: string, reset?: boolean): Observable<Game | null> {
    ...
    if (str === null || reset) {
      return new Observable((observer) => this.client.get<Game>(`${this.url}/${itemId}`).subscribe({
        next: (data) => {
